...
namespace rysq {
	...
	struct device_reference {
	    ...
	    operator T() const {
		return cuda::get(data_);
	    } 
	    device_reference& operator=(const T&value) {
		cuda::set(data_, value);
		return *this;
	    } 
	private:
	    device_ptr<T> data_;
	...
	template<typename T>
	struct block_matrix {
	    typedef cuda::device_reference<T> reference;
	    typedef cuda::device_reference<const T> const_reference;
	    typedef cuda:: device_ptr<T> pointer;
	    typedef cuda:: device_ptr<const T> const_pointer;
	    // block_matrix() : data_(wrap(NULL)) {}
	    block_matrix(const size_t (&shape)[2], const size_t (&block)[2]) {
		for (int i = 0; i < 2; ++i) {
		    shape_[i] = shape[i];
		    block_[i] = block[i];
		}
	    ...
	    const_pointer block(int i, int j) const {
		return data_ + block_offset(i,j);
	    }
