...
AS_IF([test "$CUDA_ROOT" = "/bin/false"], [
  AC_MSG_WARN([CUDA disabled])
  CUDA=""], [
  AS_IF([test -n "$CUDA_ROOT"], [
    AC_PATH_PROGS([CUDA], [nvcc], [], [CUDA_ROOT/bin])], [
    AC_PATH_PROGS([CUDA], [nvcc], [], [/usr/local/cuda/cuda/bin /usr/local/cuda/bin]dnl
