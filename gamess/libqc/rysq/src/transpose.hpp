#ifndef _RYSQ_TRANSPOSE_HPP_
#define _RYSQ_TRANSPOSE_HPP_
...
template<class Q> Q transpose(const Q &quartet, int value) {
    return cxx::utility::permute(quartet,  transpose_permutation(value));
}

inline Quartet<Shell> transpose(const Quartet<Shell> &quartet, int value) {
    boost::array<Shell*,4> p = 
	cxx::utility::permute(quartet.data(), transpose_permutation(value));
