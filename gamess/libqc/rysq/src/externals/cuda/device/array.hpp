...
namespace cuda {
	...
	struct array_base {
	    typedef host::device_ptr<T> device_ptr;
	    typedef host::device_ptr<const T> const_device_ptr;
	    array_base() : data_(NULL), size_(0) {}
	    array_base(size_t size) 
		: data_(host::malloc<T>(N*size).data()), size_(size) {}
	    array_base(device_ptr data, size_t size) : data_(data.data()), size_(size) {}
	...
	struct const_array : array_base<const T,N> {
	    ...
	    const_array(host::array_ref<const T,N> &ref)
		: base(ref.data(), ref.size()) {}
	...
	struct const_array<int4,1 > : array_base<const int4,1> {
	    ...
	    explicit const_array(size_t size) : base(size) {}
	    const_array(device_ptr data, size_t size = 0) : base(data, size) {}
	    const_array(host::array_ref<const int,4> &ref)
		: base(host::reinterpret<const int4>(ref.data()), ref.size()) {}
	...
	struct const_array<T,1> : array_base<const T,1,T&> {
	    ...
	    explicit const_array(size_t size) : base(size) {}
	    const_array(device_ptr data, size_t size = 0) : base(data, size) {}
	    const_array(host::array_ref<const T,1> &ref)
		: base(ref.data(), ref.size()) {}
