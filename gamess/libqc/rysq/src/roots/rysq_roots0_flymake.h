...
__device__ static inline double Ghondo_roots0_x33toInf(double const X) {
    return sqrt(.785398163397448 / X);
}

__device__ static inline double Ghondo_roots0_x15to33(double const X) {
    double xinv = 1/X;
    double e = exp(-X);
...
__device__ static inline double Ghondo_roots0_x10to15(double const X) {
    double e = exp(-X);
    double xinv = 1. / X;
...
__device__ static inline double Ghondo_roots0_x5to10(double const X) {
    double e = exp(-X);
    double xinv = 1. / X;
...
__device__ static inline double Ghondo_roots0_x3to5(double const X) {
    double y = X - 4.;
...
__device__ static inline double Ghondo_roots0_x1to3(double const X) {
    double y = X - 2.;
...
__device__ static inline double Ghondo_roots0_x0to1(double const X) {
...
__device__ static inline double Ghondo_Rysq_roots0(double const X) {
...
__device__ static inline double Rysq_roots0(double const X) { 
    
const double C15_33[] = { -6.0156581186480998977e-5,
				 -4.9695241464489997574e-1,
				 1.9623264149430000303e-1 };


 const double C10_15[] = { -2.1916512131607000959e-5,
				 -4.9893752514047001734e-1,
				 2.2991849164985000975e-1,
				 -1.8784686463511998666e-1 };

 const double C5_10[] = { -3.1501078774085e-6,
				-.49984072848436,
				.24645596956002,
				-.32883030418398,
				.53689283271887,
 ...
 const double C3_5[] = { .0528406320615584,
			       -.0175257821619926,
			       .00433207949514611,
			       -8.81947375894379e-4,
			       1.526537461148e-4,
  ...
  const double C1_3[] = {.115702180856167,
			      -.0529428148329736,
			      .0161723488664661,
			      -.00379490003707156,
			      7.24888732052332e-4,
  ...
  const double C0_1[] = {.333333333333318,
			      -.199999999997023,
			      .0714285713298222,
			      -.0185185172458485,
			      .00378787044215009,
	...
	if(X > 33.) {
	    ...
	    const int lsize = sizeof(C15_33)/sizeof(double);
	    ...
	    const int lsize = sizeof(C10_15)/sizeof(double);
	    for(int i = 0; i < lsize; ++i) {
		W += x1i*C10_15[i];
		x1i *= x1;
	    }
	    ...
	    const int lsize = sizeof(C5_10)/sizeof(double);
	    for(int i = 0; i < lsize; ++i) {
		W += x1i*C5_10[i];
		x1i *= x1;
	    }
