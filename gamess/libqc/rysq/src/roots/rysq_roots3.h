#ifndef _RYSQ_ROOTS3_HPP_
#define _RYSQ_ROOTS3_HPP_
...
BEGIN_NAMESPACE(rysq, roots3)

static const size_t N = 3;
    
_constant_
double R3[3] = { 0.190163509193487, 1.78449274854325, 5.52534374226326 };
_constant_
double W3[3] = { 1.0, 0.177231492083829, 0.00511156880411248 };

template<int N, int L, typename T>
__device__
static void evaluate2(const double X, double *R, const double (&C)[N][L],
		      const T &thread) {
    double p = 0.0;
    for (int i = L-1; i >= 0;  --i) {
	p = (p + C[thread][i])/X;
...
static void evaluate2(const double X, double *R, const double (&C)[N][L],
    ...
    for (int a = 0; a < N; ++a) evaluate2(X, R, C, a);
}


template<int n>
__device__
static void evaluate0(const double X, const double *C,
		      const double e, double &W) {
    W = 0.0;
    for (int i = n - 1; i > 0; --i) {
	W = (W + C[i])/X;
    }
...
template<int N>
__device__
static double evaluate0(const double X, const double (&C)[N]) {
    double p = 0.0;
    for (int i = N - 1; i > 0; --i) {
	p = (p + C[i])/X;
    }
...
namespace x0 {
    __constant__
    const double R_CP[3][2] = {{ 0.0603769246832797, -0.00928875764357368 },
			       { 0.776823355931043, -0.119511285527878 },
			       { 6.66279971938567, -1.02504611068957 }};
    __constant__
    const double W_CP[3][2] = {{ 0.467913934572691, -0.0564876917232519 },
    ...
    static void evaluate(const double X, double *R, double *W,
			 const T &thread) {
	evaluate1(X, R, W, R_CP , W_CP, thread);
    }
...
namespace x0to1 {
    _constant_
    double  R_CP[3][8] = {{ 0.060376924683281, -0.00928875764374337, 0.00102893039315878, -9.55085533670919E-5, 7.58291285499256E-6, -5.01081057744427E-7, 2.4013441570345E-8, -5.1018669153887E-10 },
    ...
    _constant_
    double F_CP[10] = { 0.199999999999986, -0.142857142854412, 0.0555555554649585, -0.015151513983854, 0.00320512054753924, -5.55526624875562E-4, 8.16324851790106E-5, -1.03463270693454E-5, 1.09552870123182E-6, -7.6091148609885E-8 };
    // __device__
    // static void evaluate(const double X, double *R, double *f) {
    //     evaluate1(X, R, R_CP);
    ...
    static void evaluate(const double X, double *R, double *f,
			 const T &thread) {
	evaluate1(X, R, R_CP, thread);
	if (master(thread)) evaluate1(X, f[1], F_CP);
    }
}

namespace x1to3 {
    _constant_
    double  R_CP[3][10] = {{ 0.0452578254679079, -0.0061121934982509, 6.03194524398109E-4, -5.13430986707889E-5, 3.86118485517386E-6, -2.60122498274734E-7, 1.56592951656828E-8, -6.55098264095516E-10, 4.85300143926755E-12, 1.44687969563318E-12 },
    ...
    _constant_
    double F_CP[12] = { 0.0529428148329709, -0.0323446977320647, 0.011384700111381, -0.00289955494844975, 5.83701488646511E-4, -9.74574633246452E-5, 1.39195169625425E-5, -1.73806555021045E-6, 1.92804632038796E-7, -1.92514145088973E-8, 1.78157031325097E-9, -1.4804423107214E-10 };
    template<typename  T>
    __device__
    static void evaluate(const double X, double *R, double *f,
			 const T &thread) {
	double y = X - 2.0;
	evaluate1(y, R, R_CP, thread);
	if (master(thread)) evaluate1(y, f[1], F_CP);
    }
}

namespace x3to5 {
    _constant_
    double R_CP[3][10] = {{ 0.0350898470729044, -0.00421006346373634, 3.70326938096287E-4, -2.87048671521677E-5, 2.026002142457E-6, -1.229940017368E-7, 7.649155832025E-9, -4.66622033006074E-10, 1.44265709189601E-11, 0.0 },
    ...
    _constant_
    double F_CP[12] = { 0.0175257821619922, -0.00866415899015349, 0.00264584212770942, -6.10614987696677E-4, 1.14504948737066E-4, -1.81353179793672E-5, 2.48749160874431E-6, -3.00873821471489E-7, 3.25336816562485E-8, -3.18111322308846E-9, 2.89147476459092E-10, -2.36788772599074E-11 };
    template<typename T>
    __device__
    static void evaluate(const double X, double *R, double *f,
			 const T & thread) {
...
namespace x5to10 {
    _constant_
    double R_CP[3][14] = {{ 0.0239112249488821, -0.00239864911618015, 1.77930381549953E-4, -1.08656008854077E-5, 7.34609900170759E-7, -4.65449552856856E-8, 1.39032379769474E-9, -4.23879635610964E-11, 1.31541892704E-11, -6.264613873998E-13, -6.736701449826E-14, 7.11884203790984E-16, 5.74429401360115E-16, 0.0 },
    ...
    _constant_
    double W0_CR[7] = { -3.1501078774085E-6, -0.49984072848436, 0.24645596956002, -0.32883030418398, 0.53689283271887, -0.69955602298985, 0.46897511375022 };
    template<typename T>
    __device__
    static void evaluate(const double X, double *R, double *W,
			 const double e, const T & thread) {
	double y = X - 7.5;
	evaluate1(y, R, R_CP, thread);
	// }
	if (master(thread)) W[0] = e*evaluate0(X, W0_CR);
    }
}

namespace x10to15 {
    _constant_
    double R_CP[3][14] = {{ 0.0153435577063174, -0.00118587782909876, 8.04427643593792E-5, -4.14222202791434E-6, 1.29454702851936E-7, -3.55578027040563E-9, 7.11372337079797E-10, -8.67286219861085E-11, 1.882093066702E-12, 5.379885121517E-13, -4.084026087887E-14, -2.77189767070441E-15, 4.4213300128309E-16, 0.0 },
    ...
    _constant_
    double W0_CR[4] = { -2.1916512131607E-5, -0.49893752514047, 0.22991849164985, -0.18784686463512 };
    template<typename T>
    __device__
    static void evaluate(const double X, double *R, double *W,
			 const double e, const T &thread) {
...
namespace x15to20 {
    _constant_
    double R_CP[3][7] = {{ -2079.70687843258, 243.517435690398, -17.3209218219175, 0.781425144913975, -0.0234112415981143, 3.57901398988359E-4, -2.43270989903742E-6 },
    ...
    _constant_
    double R_CR[3][3] = {{ 9824.41363463929, -19761.1541576986, 0.0 },
			 { -291532.335433779, 1411295.05262758, -2916691.1368102 },
			 { 51149.8390849158, -189829.509315154, 0.0 }};
    _constant_
    double W0_CR[3] = { -6.0156581186481E-5, -0.4969524146449, 0.1962326414943 };
    template<typename T>
    __device__
    static void evaluate(const double X, double *R, double *W,
			 const double e, const T &thread) {
	evaluate1(X, R, R_CP, thread);
	evaluate2(X, R, R_CR, thread);
...
namespace x20to33 {
    _constant_
    double R_CP[3][5] = {{ 164.931462413877, -18.8336409225481, 1.31099142238996, -0.0500929599665316, -4.97561537069643E-4 },
    ...
    _constant_
    double R_CR[3][1] = {{ -660.344754467191 },
			 { -6309.09125686731 },
			 { -14573.4701095912 }};
    _constant_
    double W0_CR[3] = { -6.0156581186481E-5, -0.4969524146449, 0.1962326414943 };
...
namespace x33to47 {
    ...
    __device__
    static void evaluate(const double X, double *R, double *W,
			 const double e, const double w0,
			 const T &thread) {
	evaluate1(X, R, W, R_CP, W_CP, thread);
...
__device__
static void weights3( double *R, double *W, double *f, const serial_tag&) {
    double a[2];
...
__device__
static void weights3( double *R, double *W, double *f,
...
static void evaluate(const double X, double *R, double *W,
    ...
    else if (X > 33.0) x33to47::evaluate(X, R, W, e, W0, thread);
    else if (X > 5.0) {
	...
	if (X > 3.0) x3to5::evaluate(X, R, f, thread);
	else if (X > 1.0) x1to3::evaluate(X, R, f, thread);
	else x0to1::evaluate(X, R, f, thread);
	if (master(thread)) {
	    f[0] = ((X + X)*f[1] + e)/3.0;
	    W[0] = (X + X)*f[0] + e;
	}
    ...
    if (X > 33.0) return;
    if (X > 3e-7) weights3(R, W, f, thread);
...
END_NAMESPACE(rysq, roots3)

#endif // _RYSQ_ROOTS3_HPP_
