...
@REM
@REM April 27, 2010 :: Sarom Sok :: Modified to work with Windows Server 2008 HPC Edition
@REM                               (current release)
...
@REM
@SET ERICFMT=%UNC_GAMESSDIR%\ericfmt.dat
@SET MCPPATH=%UNC_GAMESSDIR%\mcpdata
@SET  EXTBAS=/dev/null
@SET  NUCBAS=/dev/null
@REM
...
@IF %PPN%==0 (
  ...
  IF %GDDIJOB%==TRUE (
    ...
    @job add  %JOBID% /name:"GAMESSGDDIJOBNodePreparation"   ^
     ...
     mpiexec -n %NODESNEEDED% %UNC_BATCHDIR%\gamess.start.gddi.bat
    @REM
    ...
    @REM
     job add  %JOBID% /name:"RunGAMESS"                        ^
	                  /depend:"GAMESSGDDIJOBNodePreparation"   ^
                      /numnodes:%NODESNEEDED%                  ^
    ...
    @REM
     job add  %JOBID% /name:"GAMESSJobNodePreparation"       ^
                      ...
                      /env:JOBNAME=%JOB%                     ^
                      /env:LOGFILE=%UNC_GAMESSDIR%\%LOGFILE% ^
     mpiexec -n %NODESNEEDED% %UNC_BATCHDIR%\gamess.start.bat
    @REM
    @REM  Task :: Run the job.
    @REM
    @ECHO.
    ...
    @REM
     job add  %JOBID% /name:"RunGAMESS"                        ^
	                  /depend:"GAMESSJobNodePreparation"       ^
                      /numnodes:%NODESNEEDED%                  ^
                      /workdir:%UNC_GAMESSDIR%                 ^
                      /stdout:%LOGFILE%                        ^
                      /stderr:%ERRFILE%                        ^
  ...
  IF %GDDIJOB%==TRUE (
    ...
    @REM
     job add  %JOBID% /name:"GAMESSJobNodeCleanup"           ^
                      ...
                      /env:JOBNAME=%JOB%                     ^
                      /env:LOGFILE=%UNC_GAMESSDIR%\%LOGFILE% ^
     mpiexec -n %NODESNEEDED% %UNC_BATCHDIR%\gamess.finish.gddi.bat
    @REM
    @ECHO =========================================================================
  ) ELSE (
    @REM
    ...
    @REM
     job add  %JOBID% /name:"GAMESSJobNodeCleanup"           ^
                      ...
                      /env:JOBNAME=%JOB%                     ^
                      /env:LOGFILE=%UNC_GAMESSDIR%\%LOGFILE% ^
     mpiexec -n %NODESNEEDED% %UNC_BATCHDIR%\gamess.finish.bat
    @REM
    @ECHO =========================================================================
  )
  @REM
